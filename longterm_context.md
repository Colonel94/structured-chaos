...
> - **Verified:** `App.test.tsx` passes unmodified; tsc + vitest + `pnpm build` green; nabu-ui-tested desktop 1440 + mobile 390, **0 console errors, no overflow**, incl. the flagged case, the one-key correction picker, provenance, and the tuning modal. Files: `ui/src/{App.tsx,main.tsx,styles.css}` + `ui/DESIGN.md`. **Demo tenant was seeded for the shots then RESET pristine** (0 cases, 6 orders) — still external-gate-ready.
> - **Follow-ons if the owner wants (NOT started):** a dark-theme toggle is now cheap (token swap); could further simplify by collapsing "Other details found"/JSON behind disclosures; the new-case modal is still ~17s inline.
>
