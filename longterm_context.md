...
> ### ⇢ SESSION HANDOFF — read this box first (updated 2026-08-19: category work DONE-FOR-NOW at **extract-v20**; **PHASE 5 FUNCTIONALLY COMPLETE**. Category journey: off-finance probe→fix v17→v19 (service↔delivery + product↔safety discriminators, combined 73.6→75.9%), then the STRUCTURAL fix **v20** (owner): service_fault made EXPLICITLY RESIDUAL ("mishandled AND no other primary harm applies") + dominance rule THE BLOCK WINS (access_availability is a checkable STATE, beats arguable service conduct). **v20: CFPB 78→82% · multidomain 73→71% · combined 75.9→77.3%, record_accuracy 31/31** — access→service bleed 5→1 CFPB / 3→0 MD. Integrity: v20 rule was fixed BEFORE re-scoring, kept regardless (it rose). **Category is DONE-FOR-NOW** — the close-call gold is owner-authored, so more prompt grinding optimises toward label inconsistency (a GOLD ceiling, not a model ceiling). **PHASE 5 IN PROGRESS (lever moved extraction→elicitation): riskiest-assumption spike DONE** (silent object-match safe: deterministic exact-key+phone resolver PASSES as a PAIR — rate 62%≥60%, accuracy 0/432 wrong→95% bound 0.7%; scaled pair-measurement CAUGHT+FIXED a wrong-bind defect via id↔phone cross-check). **OBJECT-STORE INGEST BUILT (migration 0014, `3bc43ed`):** `object_record`+`object_key` (RLS'd, schema-agnostic jsonb, pgvector HNSW, composite-FK); `app/resolve/` = keys(shared normalise+hash) · profile(stats-before-semantics identifier detection) · ingest(idempotent, batched embed) · resolver(the spike policy against the store, id↔phone cross-check → contradiction=confirm). Store API: upsert_object_record/add_object_key/resolve_object_by_key/nearest_objects/set_object_embedding/get_object. **ELICIT STAGE BUILT (`6eb8ac0`) — the anchor+2 budget enforced IN CODE:** `app/elicit/` = policy(PURE deterministic decision: anchor→desired_outcome→one clarifying drill; angry+incomplete→handoff; NEVER a 4th question; never re-ask a present field) + stage(chains off extract, resolves the anchor→CONFIRMS looked-up facts, persists case_state + increments `question_count` only when asking, idempotent on the EXTRACTED STATE so replays don't re-ask; budget accrues durably across turns on `case_record.question_count`). Store: get_case_elicit_state/get_field_values/apply_elicitation (plan in `external_mappings.elicit`). Extract now transactionally enqueues elicit (chain proven by a test); `pipeline.elicit` task. **OBJECT-SNAPSHOT-ON-BIND + CONTRADICTS ROLE DONE (`5f60a53`):** `app/resolve/snapshot.py` freezes the bound object as an immutable `source_document(doc_kind='object_snapshot')` (idempotent on the canonical state); `app/resolve/contradiction.py` = grounded/conservative LLM complaint-vs-record detector (reports only a direct checkable disagreement on a real field, else empty; surfaced never acted). Resolver now flags `is_contradiction` (id≠phone). Elicit on bind → snapshot + `contradicts` citations (deterministic anchor-mismatch on anchor_value; LLM record-vs-complaint on fault); blob+llm injected (budget stays deterministic), queue passes the real ones. Store: get_case_channel, get_latest_extraction_id. **CHANNEL LOOP DONE (`b0584cb`) — the conversation is closed end-to-end:** `Channel` interface + `channel_backend` config-switch (LOCAL = $0 record-and-relay `LocalChannel`; CLOUD = WhatsApp, deferred/raises until Meta test number; `FakeChannel` for tests) + `get_channel`. Migration 0015 `outbound_message` (RLS'd audit + idempotency, UNIQUE(tenant,case,question_hash) → send once). `app/channel/dispatch.py` = claim→send→record in one txn; no pending question / no recipient → no-op. Elicit chains dispatch transactionally when it asks (`pipeline.dispatch`); reply re-enters intake→extract→elicit and advances the drill. **Fix (surfaced by the loop): `has_anchor` = stated anchor OR resolved object — the sender phone alone doesn't count (may not be the number used to order), so unresolved cases correctly still ask the anchor.** **PHASE 5 IS NOW FUNCTIONALLY COMPLETE** (object store + resolution + snapshot/contradicts + anchor+2 elicitation + the send/reply loop). **NEXT = Phase 6:** confidence/abstention (self-consistency + calibration + selective-prediction routing) + the DETERMINISTIC SLA/priority/routing rules engine (YAML) + clock-from-first-contact. NEW MEMORY [[report-metric-pairs-and-n]]. **16 migrations, 134 tests +1 skip, ruff/black/mypy clean (72 files). Prompt at extract-v20.** Known PoC limits: phone normalisation digits-only (no +44-vs-0 unify, libphonenumber deferred); live WhatsApp transport deferred (Meta test number, Gate A) — the loop is proven with LocalChannel/FakeChannel.) The dated UPDATE blocks below are the AUDIT TRAIL ONLY (historical, newest first — the Phase-4 remediation R0–R7, the convergence retraction, the category v13–v20 journey); **THIS box is the current truth and the live frontier — start Phase 6 from here.** Dip into the dated blocks only to understand WHY a past decision was made.
>
