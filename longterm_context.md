...
>
> **GIT:** `HEAD @ 7184d91`, review-usability + feedback loop + tuning digest/drafter + open-as-PR + gated eval + **orphaned-job reaper**, **pushing this session**. **22 migrations, 270 tests +1 skip**, ruff/black/mypy(app) + UI tsc + vitest clean. **Live stack UP** (engine :8000, ONE worker on new code, review UI :5173, tunnel was up for the voice test — see RUN IT). **VOICE-FIRST PROVEN on a real iPhone.** Demo tenant has test corrections/feedback + 2 voice cases (digest populated — reseed before external gate). **Demo tuning PR OPEN: [#1](https://github.com/Colonel94/structured-chaos/pull/1)** (DRAFT/do-not-merge — owner can close).
>
