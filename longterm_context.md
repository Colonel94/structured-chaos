...
> - **⚠ DEMO TENANT NOT pristine right now** — re-seeded with 6 review cases (`seed_review_cases.py`) so the owner could see the populated look in both themes. **RESET before the external gate:** `reset_demo_tenant.py "Portal Demo Co"` → `seed_portal_orders.py "Portal Demo Co"` (back to 0 cases / 6 orders).
