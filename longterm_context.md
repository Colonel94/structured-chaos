...
*Last updated: 2026-08-21b (session: portal shakedown + the "meaningful resolution" arc — status-link
fix · "what happened" drill · analytical drill (Moment 3) + object store · case synthesis + category
routing + contradiction-detector fix. All pushed, verified live. Standing convergence caveat still
holds: composite curve flat ~90% hapax → convergence UNPROVEN.)*
...
> - **KNOWN minor polish (not blockers):** the analytical-drill confirmation lists the first 4 non-contact record facts in record order (states "customer name …", can hit the 4-cap before `delivery_slot`) — would be better to prioritise anomaly-bearing fields; the synthesis summary is deterministic (good for trust) not LLM-fluent. **Deeper root still open (owner's call):** the extractor's grammar forces `fault` non-null (unlike `desired_outcome`) so it fabricates on contentless input — the elicit-layer grounding gate catches it, but the clean fix is making `fault` nullable + prompting abstention (needs an eval re-run, deferred).
>
...
>
> **NEXT STEPS (prioritized) — pick up here:**
> 1. **[owner action] Push the 4 unpushed commits** (WhatsApp + portal) if wanted — offer it first thing.
> 2. **[human/owner] Put `eval/holdout_labels.csv` in front of an independent labeller** (neither me nor owner) — THE binding numbers constraint; unblocks the confidence gate + true category ceiling + the ACCURACY side of §4 (the drill+trust side is already measured & strong). Instructions ready (`eval/holdout_labels_INSTRUCTIONS.md`); when it returns, score v20 against it for the first non-self-graded accuracy.
> 3. **[owner action] Go live on the two channels** (both BUILT, need owner-only setup): WhatsApp = the Meta test-number setup (`docs/WHATSAPP-SETUP.md`, ~20min identity-gated) then message it; Portal = run engine with `PORTAL_ENABLED=true PORTAL_SECRET=…`, `scripts/portal_enable.py` to mint a key, open `/p/s/<key>`.
> 4. **[I own, unblocked] Sparse-slice elicitation + the real object-match RATE** — the two §4 rows the live run couldn't close: (a) build/label ≥20 too-sparse-to-act cases so sparse→actionable + the 2nd drill are stressed (the 216 full narratives never trigger them); (b) the real ≥60% silent-match RATE needs a real anchored-complaint distribution. Plus e2e latency under load + backfill-correctness.
> 5. **Convergence remediation (the moat)** — needs richer real recurring data + the dedup/mint proof to bend the composite curve, or the honest retraction stands.
> 6. **Arabic** (separate project) — the differentiator; needs the Gulf voice corpus (track T3).
...
>
> **RUN IT (demo next session):** `docker compose -f deploy/docker-compose.yml up -d db minio` (DB+minio already healthy this session); Ollama up (qwen3:14b — start `Ollama app.exe` after reboot); engine `cd engine && uv run uvicorn app.main:app --port 8000`; UI `cd ui && pnpm dev`; open `http://localhost:5173/?tenant=<uuid>`. **Review-UI demo tenant `cd710492-19f6-4dbd-a921-27ca09e3b5f2`** (15 cases + orders; note: I committed one of its cases + left a partial `Empty-Demo-UItest` throwaway this session — both dev-only). **PORTAL:** run engine with `PORTAL_ENABLED=true PORTAL_SECRET=<any>`, `uv run python scripts/portal_enable.py "Co" http://localhost:5173`, open `/p/s/<key>` (demo tenant `Portal Demo Co` = `a17456c2-6dbc-492a-9128-7747f8650196`, key `ek_ZDw3qSBD1pqXowBRoSQizyNE`). **WhatsApp:** `docs/WHATSAPP-SETUP.md`. **The compose engine/migrate IMAGES carry stale code (Docker COPY-cache) — `docker compose build --no-cache engine` before shipping the container.** Windows host: keep `PYTHONIOENCODING=utf-8` for non-ASCII scripts; two stray dev engines may still be running (:8000 old code pre-this-session's routers, plus a possible :8001) — restart :8000 with the new code (portal/whatsapp routers) to serve them.
>
