...
> ## ✅ CURRENT STATE + NEXT STEPS (2026-08-21 — the clean top-of-mind summary; the dated SESSION HANDOFF notes below are session detail + audit trail)
>
> **BUILT & VERIFIED (phases 0–7 done, trust spine strong):** RLS tenant isolation, immutable originals, idempotent replay, per-value provenance, the human-approval COMMIT GATE, per-field provenance spans (sentence / audio-segment / image-bbox), the deterministic priority/SLA/routing rules engine, calibrated per-field confidence (honest — `gate_met=False`, everything routes to review), the <30s keyboard review UI + WeasyPrint report. **`origin/main @ 549c988` (local; the two eval commits below are unpushed); 185 tests +1 skip; ruff/black/mypy + UI tsc/vitest clean.**
>
...
> 1. **[human/owner] Put `eval/holdout_labels.csv` in front of an independent labeller** (neither me nor owner) — the binding numbers constraint; unblocks the confidence gate + the true category ceiling + the ACCURACY side of the §4 scorecard (the drill+trust side is already measured & strong). Harness + instructions ready (`eval/holdout_labels_INSTRUCTIONS.md`); when it returns, score v20 against it for the first non-self-graded accuracy.
> 2. **[I own, unblocked] Sparse-slice elicitation + the real object-match RATE** — the two §4 rows the live run couldn't close: (a) build/label the ≥20 too-sparse-to-act cases (four-useless-words) so sparse→actionable + the 2nd-drill budget are actually stressed (the 216 full narratives never trigger them); (b) the real ≥60% silent-match RATE needs a real anchored-complaint distribution (constructed mixes only give resolver-quality, which is already 100% recall / 0 wrong). Plus end-to-end latency under load + backfill-correctness.
