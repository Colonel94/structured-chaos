...
>
> **GIT:** `HEAD @ 6a5e331`, review-usability feature committed (§0 doc commit follows), **local — PUSH PENDING**. **21 migrations, 256 tests +1 skip**, ruff/black/mypy(app) + UI tsc + vitest clean. **Live stack UP** (engine :8000 restarted on new code, review UI :5173 — see RUN IT). Demo tenant has 6 sentiment/test cases + 1 stray review_event from live verification (reseed before the external gate).
>
