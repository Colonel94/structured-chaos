...
>
> **GIT:** `HEAD @ ab49660`, review-usability surface + per-field breakdown + **the feedback loop** + demo seed, **pushing this session**. **22 migrations, 260 tests +1 skip**, ruff/black/mypy(app) + UI tsc + vitest clean. **Live stack UP** (engine :8000 restarted on newest code, review UI :5173 — see RUN IT). **Demo tenant PRISTINE for the reviewer**: `Portal Demo Co`, 6 orders + **6 realistic seeded cases** (`seed_review_cases.py`), 0 feedback, review-time median cleared.
>
