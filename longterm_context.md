...
> - **DEMO TENANT is PRISTINE + external-gate-ready** (reset 2026-08-25 after the redesign shots: 0 cases, 6 orders, review-stats cleared). It was briefly seeded with 6 review cases to view the populated look in both themes, then reset. Reseed for viewing anytime with `seed_review_cases.py "Portal Demo Co"`; reset with `reset_demo_tenant.py` → `seed_portal_orders.py`.
