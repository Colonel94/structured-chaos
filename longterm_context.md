...
> ## ✅ CURRENT STATE + NEXT STEPS (2026-08-20 — the clean top-of-mind summary; the dated SESSION HANDOFF notes below are session detail + audit trail)
>
> **BUILT & VERIFIED (phases 0–7 done, trust spine strong):** RLS tenant isolation, immutable originals, idempotent replay, per-value provenance, the human-approval COMMIT GATE, per-field provenance spans (sentence / audio-segment / image-bbox), the deterministic priority/SLA/routing rules engine, calibrated per-field confidence (honest — `gate_met=False`, everything routes to review), the <30s keyboard review UI + WeasyPrint report. **`origin/main @ 372d878` (pushed); 185 tests +1 skip; ruff/black/mypy + UI tsc/vitest clean.**
>
...
>
> **HONEST OPEN GAPS (a sellable SURFACE is not a passing SCORECARD — from the full audit + owner review):**
> - **Convergence (the moat) FAILS at its committed metric** — 7.6% dup (>5%), ~90% hapax; unproven on real data.
> - **§4 accuracy gates unmet** — governed core ~63–71% (gate ≥95%), category 74–82% (≥90), auto-route `gate_met=False` (≥98).
> - **THE BINDING CONSTRAINT:** an INDEPENDENT human-labelled held-out slice (60–80 cases, neither me nor owner) — it gates the confidence number, the category score, AND the review ordering; a better extractor can't beat my own label inconsistency ([[calibration-label-ceiling-per-class]]). Owner's data-source call; I can generate the blind sheet on go.
> - **Arabic entirely absent (0/216)** — the marquee differentiator is unbuilt + unmeasured (separate project).
> - **Most §4 metrics UNMEASURED** — Phase 8 (the threshold scorer over the ground-truth set) not started.
> - **Cloud path partly vaporware** — `asr_cohere`/`embed_bge` modules missing; WhatsApp send is a stub.
>
> **NEXT STEPS (prioritized):**
> 1. **[human/owner] Independent human-labelled held-out slice** — the binding numbers constraint; unblocks #2 + the confidence gate. I prep the blind sheet on go (`eval/make_label_sheet.py`); owner picks re-label-existing-216 vs fresh T3 data.
> 2. **Phase 8 — the §4 threshold scorer** over the ground-truth set: measure the unmeasured (zero-edit rate, ambiguous-flagged, sparse→actionable, review time, end-to-end latency, object-match pair).
> 3. **Async intake** — replace the ~17s inline spinner with a background job + progress (UX; not blocking).
> 4. **Convergence remediation (the moat)** — needs richer real recurring data + the dedup/mint proof to bend the composite curve, or the honest retraction stands.
> 5. **Arabic** (separate project) — the differentiator; needs the Gulf voice corpus (track T3).
> - Deferred (wrong lever now): the cloud extractor (until #1); live WhatsApp transport (Meta test number); the promised-vs-actual confirmation phrasing (per-tenant field-role pairing).
>
> **RUN IT (demo next session):** `docker compose -f deploy/docker-compose.yml up -d db minio`; Ollama up (qwen3:14b); engine `cd engine && uv run uvicorn app.main:app --port 8000`; UI `cd ui && pnpm dev`; open `http://localhost:5173/?tenant=<uuid>`. Seed: `scripts/seed_review_demo.py`. Demo tenant this session: `cd710492-19f6-4dbd-a921-27ca09e3b5f2` (cases + orders loaded). **The compose engine/migrate IMAGES carry stale code (Docker COPY-cache on the "Structured Chaos" space) — `docker compose build --no-cache engine` before shipping the container.** Windows host: the logging 500 is fixed, but keep `PYTHONIOENCODING=utf-8` for readable non-ASCII scripts.
>
