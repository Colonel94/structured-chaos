...
>
> **GIT:** `HEAD @ d93c1b3`, review-usability surface + breakdown + feedback loop + tuning digest + drafter + **open-as-PR + gated eval**, **pushing this session**. **22 migrations, 268 tests +1 skip**, ruff/black/mypy(app) + UI tsc + vitest clean. **Live stack UP** (engine :8000, review UI :5173 — see RUN IT). **Demo tenant PRISTINE for the reviewer**: `Portal Demo Co`, 6 orders + **6 realistic seeded cases**, 0 feedback/corrections, digest cleared. **Demo tuning PR OPEN: [#1](https://github.com/Colonel94/structured-chaos/pull/1)** (branch `tuning/prompt-delta-20260824-131802`, DRAFT/do-not-merge — owner can close).
>
