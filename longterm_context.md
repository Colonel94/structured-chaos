...
> 1. **[human/owner] Put `eval/holdout_labels.csv` in front of an independent labeller** (neither me nor owner) — the binding numbers constraint; unblocks the confidence gate + the true category ceiling. Harness + instructions ready (`eval/holdout_labels_INSTRUCTIONS.md`); when it returns, score v20 against it for the first non-self-graded accuracy.
> 2. **[I own, unblocked] Live elicitation + object-match run** — turns ~8 of the UNMEASURED §4 rows into real numbers with the existing 216 set + DB/Ollama up (`eval/spike_entity_resolution.py` + an elicitation pass over the sparse slice). Report object-match as a PAIR.
