...
>
> **GIT:** `HEAD @ de46251`, review-usability surface + per-field breakdown, **pushing this session**. **21 migrations, 257 tests +1 skip**, ruff/black/mypy(app) + UI tsc + vitest clean. **Live stack UP** (engine :8000 restarted on newest code, review UI :5173 — see RUN IT). **Demo tenant RESET + RESEEDED** (`Portal Demo Co`, 6 orders, 0 cases, review-time median cleared) — clean for the reviewer; the stray review_event is gone.
>
